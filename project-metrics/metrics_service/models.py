...
class MetricResult(Base):
  ...
  __table_args__ = (sqlalchemy.Index(
    'computed_at__name', 'computed_at', 'name'),)
  ...
  # PRs, a percentage value [0-100], latency in minutes, etc. The metric itself
  # shall be responsible for interpreting the value, determining the
  # corresponding score, and rendering the result.
  value = sqlalchemy.Column(sqlalchemy.Integer)  # type: int
  computed_at = sqlalchemy.Column(
    sqlalchemy.DateTime, default=datetime.datetime.utcnow)  # type: datetime.datetime

  def __repr__(self) -> Text:
    return "<Metric(name='%s', value='%d', computed_at='%s')>" % (
